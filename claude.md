...
- **Claude Desktop/Web (remote)**: Point Claude to your HTTPS deployment (for example `https://mcp.example.com`). Claude fetches `/.well-known/mcp.json`, registers a client with redirect `https://www.claude.ai/api/auth/callback`, and walks through OAuth automatically.
...
- **ChatGPT Developer Mode**: Settings → Connectors → Advanced → Developer Mode → Add your HTTPS endpoint (or `http://127.0.0.1:8000/mcp` for local testing). The MCP server now recognises ChatGPT redirects such as `https://chat.openai.com/aip/api/auth/callback`, eliminating the "invalid redirect" error.
