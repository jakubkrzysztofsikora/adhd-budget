...
class MCPApplication:
    ...
    def _ensure_enable_banking(self) -> EnableBankingService:
        if not self._enable_banking_ready():
    ...
    async def oauth_authorize(self, request: web.Request) -> web.StreamResponse:
        ...
        if not self._enable_banking_ready():
            LOGGER.warning(
                "Enable Banking credentials missing - issuing authorization code without consent"
            )
            return self._issue_direct_authorization_response(
                request,
                client_id=client_id,
                redirect_uri=redirect_uri,
                scope=scope,
                ...
                resource=resource,
                notice=(
                    "Enable Banking credentials are not configured; returning a limited OAuth "
                    "token without bank data access. Configure ENABLE_APP_ID and "
                    "ENABLE_PRIVATE_KEY_PATH for full functionality."
                ),
            )
    ...
    def _issue_direct_authorization_response(
        self,
        request: web.Request,
        *,
        client_id: str,
        ...
        body_notice = notice or "Continue back to your connector to finish setup."
        html_body = textwrap.dedent(
            f"""
            <!doctype html>
            <html>
                ...
                <body>
                    ...
                    <p>{body_notice}</p>
                    <p><a href="{location}">Return to connector</a></p>
                </body>
            </html>
            """
        ).strip()
