    ...
    async def rpc_tools_call(self, params: Dict[str, Any], request: web.Request, session: Session) -> Dict[str, Any]:
        ...
        if name in self.protected_tools:
            ...
            if auth_header:
                ...
                access_token = session.metadata.get("oauth_token")

            if not access_token:
                raise web.HTTPUnauthorized(text="Missing bearer token")

            token_info = self.oauth.validate_bearer(access_token)
            request["oauth_token_info"] = token_info
            request["oauth_access_token"] = access_token
            if session:
                session.metadata["oauth_token"] = access_token
                session.metadata["oauth_token_info"] = token_info
